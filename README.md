# MMvIB Registry
This project provides a registry for Model Adapters. Such that Model Handlers can find them.
It uses currently an in-memory database, so restarting will lose all data in the registry.

The API is available at http://localhost:9200/openapi, including the schema's used for the ModelAdapter description.
